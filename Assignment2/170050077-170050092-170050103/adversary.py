...
def floodingThreadTask(miner, n):
    """ Floods random blocks to 'n' miners with 100 blocks per interArrivalTime frequency """
    while True:
        time.sleep(0.01 * interArrivalTime)
        floodlist = miner.peerList[0:n] if (len(miner.peerList) >= n or len(miner.peerList) <= 0) else miner.peerList
        try:
            msg = bytearray(os.urandom(8))
            for peer in floodlist:
                peer.socklock.acquire()
                peer.socket.send(msg)
                peer.socklock.release()
        except Exception as e:
            peer.socklock.release()
            print("Error in sending random blocks --- %s" % e)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--numflood")
    ...
    # Starts flooding attack
    threading.Thread(
        target=floodingThreadTask,
        args=(miner,numflood)
    ).start()
